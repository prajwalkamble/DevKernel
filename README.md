...
A hand-drawn animation of merge sort is a drawing of what somebody *remembers* merge sort doing, and it will be subtly wrong in exactly the places a learner is trying to understand. So every visualization in `lib/visuals` is an ordinary implementation with `emit()` calls threaded through it. The frames are a side effect of the algorithm actually executing.
...
| --- | --- | --- |
| `npm run verify` | Types, lint, and every visualization's frames — a few seconds | node |
